			...
			if (uniqueKeys.length < minNumOfBins) {
				...
				do {
					...
					if (binType === "hexagon") {
						...
						binner = new Binner().radius(binRadius).extent([
							[0, 0],
							[1, 1],
						]); //extent from [0, 0] to [1, 1] since we already normalized data.
						bins = binner.hexbin(normalizedPoints);
					} else if (binType === "rectangular") {
						// This section uses hexagon binning
						let binWidth = 1 / binSize;
						binRadius = binWidth;
						binner = new RectangularBinner(
							normalizedPoints,
							binSize
						);
					...
					bins.length > maxNumOfBins ||
					bins.length < minNumOfBins
				);
			}
